 ...
 * Pre-hydration theme boot script.
 *
 * v0.9.1 round-5 fix: if localStorage throws (privacy mode, sandbox,
 * disabled cookies), STILL honor prefers-color-scheme rather than
 * silently defaulting to light. The outer try wraps the storage read
 * only; matchMedia + setAttribute run unconditionally.
 *
 * Hand-minified to stay one-line in View Source on production —
 * Next's dangerouslySetInnerHTML doesn't auto-minify.
 */
const themeBootScript =
  \`(function(){var t=null;try{t=localStorage.getItem('helix-theme');}catch(e){}var d=t==='dark'||(t==null&&window.matchMedia&&window.matchMedia('(prefers-color-scheme: dark)').matches);document.documentElement.setAttribute('data-theme',d?'dark':'light');})();\`;
...
export function ThemeToggle() {
  ...
  useEffect(() => {
    // Read what the boot script applied. Fallback chain matches the
    // boot script's logic so behavior is consistent if the boot script
    // somehow didn't run (CSP block, etc.):
    //   1. data-theme already set on <html>
 ...
 *
 * v0.9.1 round-5 fix: if localStorage throws (privacy mode, sandbox,
 * disabled cookies), still honor prefers-color-scheme. The try wraps
 * the storage read only; matchMedia + setAttribute run regardless.
 */
(function () {
  var t = null;
  try {
