 ...
 * functions parameterized by the consumer's `dsName` (e.g. `aurora` so
 * the page emits `<aurora-card>`) and `dsClass` (PascalCase, e.g.
 * `AuroraCard`).
 *
 * Adaptation rules (per shimmying-roaming-kernighan plan, Phase 2):
 *
 * 1. Tag substitution — every literal `hx-card`, `hx-checkbox` etc. body
 *    reference is replaced with the consumer's `{ds}-card`, `{ds}-checkbox`
 *    etc. References to upstream `@helixui/library` import paths stay
 *    LITERAL (the consumer's package manager pulls @helixui/library, then
 *    their scaffolded `client-element.ts` extends it under their own
 *    namespace).
 * 2. NO `?raw` imports of `AAA-AUDIT.md` — the InlineAuditPanel from
 *    Phase 1 renders null without a `markdown` prop. Audit content is
 *    opt-in via consumer wiring.
 * 3. NO `<AAAConformanceCard>` — dropped in the prior phase.
 * 4. NO `<SectionHead>` import — that helper was not lifted in Phase 1.
 *    The MDXes here use plain `###` headings instead.
 * 5. Title taxonomy: `Components/{DsClass}Card/Conformance` (matches the
 *    existing button MDX pattern in scaffold.ts ~line 10760).
 * 6. Composition: `<APGPatternCard>` + `<ConsumerObligations>` only.
 *    `<A11yStatusCard>` is auto-injected by HelixDocsPage at runtime;
 *    DO NOT add explicit JSX for it.
...
{/* ${dsName}-card.mdx — AAA conformance documentation for ${dsName}-card */}
...
A primary use of ${dsName}-card: a dashboard summary block. The card owns
its own elevation, padding, and slot composition; consumers control only
the inner content.
...
<div className="hx-narrative-hero">
  ...
  <${dsName}-card>
    ...
    <div slot="footer">
      <${dsName}-button variant="primary">Open inbox</${dsName}-button>
    </div>
  </${dsName}-card>
</div>
...
<APGPatternCard tag="${dsName}-card" />
...
{/* ${dsName}-checkbox.mdx — AAA conformance documentation for ${dsName}-checkbox */}
...
<div className="hx-narrative-hero">
  ...
  <div role="group" aria-label="Sign-up acknowledgements" style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
    <${dsName}-checkbox name="tos" required>
      I agree to the Terms of Service and Privacy Policy.
    </${dsName}-checkbox>
    <${dsName}-checkbox name="updates">
      Email me product updates and release notes.
    </${dsName}-checkbox>
    <${dsName}-checkbox name="research">
      Include my account in anonymized usage research.
    </${dsName}-checkbox>
  </div>
  <div className="hx-narrative-hero-actions">
    <${dsName}-button variant="ghost">Cancel</${dsName}-button>
    <${dsName}-button variant="primary" type="submit">Create account</${dsName}-button>
  </div>
</div>

## ARIA + keyboard contract

<APGPatternCard tag="${dsName}-checkbox" />
...
{/* ${dsName}-dialog.mdx — AAA conformance documentation for ${dsName}-dialog */}
...
The most consequential deployment of ${dsName}-dialog: a destructive
...
<div className="hx-narrative-hero">
  ...
  <${dsName}-dialog open aria-labelledby="delete-project-title" aria-describedby="delete-project-desc">
    ...
    <div slot="footer" className="hx-narrative-hero-actions">
      <${dsName}-button variant="ghost">Cancel</${dsName}-button>
      <${dsName}-button variant="danger">Delete project</${dsName}-button>
    </div>
  </${dsName}-dialog>
</div>
...
<APGPatternCard tag="${dsName}-dialog" />
...
// hx-form → {ds}-form
//
// hx-form references multiple sibling atoms (text-input, select, checkbox,
// button). All of them get the dsName prefix so the emitted MDX shows a
// coherent {ds}-* family rather than mixing in literal hx-* tags.
// ---------------------------------------------------------------------------
...
{/* ${dsName}-form.mdx — AAA conformance documentation for ${dsName}-form */}

<Meta title="Components/${dsClass}Form/Conformance" />

# ${dsClass} Form — AAA conformance

> WCAG 2.2 Level AAA target including 3.3.6 Error Prevention (All). Coordinates the validity state of its child form atoms (${dsName}-text-input, ${dsName}-select, ${dsName}-checkbox), emits aggregated submit / reset / change events, and surfaces a single accessible error summary above the field list when validation fails.
...
<div className="hx-narrative-hero">
  ...
  <${dsName}-form>
    ...
    <${dsName}-checkbox name="tos" required style={{ display: 'block', marginBottom: '16px' }}>
      I agree to the Terms of Service.
    </${dsName}-checkbox>
    <div className="hx-narrative-hero-actions">
      <${dsName}-button variant="ghost" type="reset">Reset</${dsName}-button>
      <${dsName}-button variant="primary" type="submit">Create account</${dsName}-button>
    </div>
  </${dsName}-form>
</div>
...
<APGPatternCard tag="${dsName}-form" />
...
<ConsumerObligations
  tag="${dsName}-form"
  obligations={[
    'Use the form atoms (${dsName}-text-input, ${dsName}-select, ${dsName}-checkbox) as children, not raw <input> / <select>. Each atom owns its own label / helper-text / error-message scaffolding wired to the parent form via ElementInternals; raw inputs break the validity aggregation.',
...
${dsName}-dialog (form-in-modal) lives in the \`Components/${dsClass}Form\`
story tree.
`,
  };
}
...
{/* ${dsName}-select.mdx — AAA conformance documentation for ${dsName}-select */}
...
<div className="hx-narrative-hero">
  ...
  <${dsName}-select label="Time zone" name="tz" required>
...
<APGPatternCard tag="${dsName}-select" />
...
<ConsumerObligations
  ...
  obligations={[
    ...
    'Use real <option> elements (or the matching ${dsName}-option child if your component shape requires it) — do not render a generic <div> menu. Native option semantics provide selection-state announcements, type-ahead, and APG combobox roles for free.',
...
disabled-option patterns, and integration with ${dsName}-form lives in
...
{/* ${dsName}-tabs.mdx — AAA conformance documentation for ${dsName}-tabs */}
...
A canonical use of ${dsName}-tabs: settings-panel navigation. Three
peer panels, each with a clear name and a focused tab indicator. Tab
into the tablist and use <kbd>ArrowRight</kbd> / <kbd>ArrowLeft</kbd>
to move the cursor, <kbd>Enter</kbd> to switch panels.

<div className="hx-narrative-hero">
  <h3>Account settings</h3>
  <${dsName}-tabs>
    <${dsName}-tab slot="tab" panel="profile">Profile</${dsName}-tab>
    <${dsName}-tab slot="tab" panel="notifications">Notifications</${dsName}-tab>
    <${dsName}-tab slot="tab" panel="security">Security</${dsName}-tab>
    <${dsName}-tab-panel name="profile">
      <p>Display name, avatar, time zone, and language.</p>
    </${dsName}-tab-panel>
    <${dsName}-tab-panel name="notifications">
      <p>Email digests, in-app alerts, and push preferences.</p>
    </${dsName}-tab-panel>
    <${dsName}-tab-panel name="security">
...
<APGPatternCard tag="${dsName}-tabs" />
...
<ConsumerObligations
  tag="${dsName}-tabs"
  obligations={[
    'Provide a meaningful name on every ${dsName}-tab via slot text. The accessible name is the tab label, and the screen-reader announcement is the only navigation aid for non-sighted users moving between panels.',
    'Match every ${dsName}-tab\\'s panel attribute to a ${dsName}-tab-panel\\'s name attribute. The component wires aria-controls + aria-labelledby automatically, but only if the names match — a typo silently breaks the screen-reader association.',
    ...
    'Use ${dsName}-tabs for peer-content navigation (settings categories, dashboard sections), not for primary site navigation. For site nav, use a ${dsName}-top-nav or ${dsName}-side-nav component — those expose the URL-route + visited-state surface tabs do not.',
...
size modifiers, scrollable tablists, and integration with ${dsName}-card
and ${dsName}-form lives in the \`Components/${dsClass}Tabs\` story tree.
`,
  };
}
...
{/* ${dsName}-text-input.mdx — AAA conformance documentation for ${dsName}-text-input */}
...
A canonical use of ${dsName}-text-input: the sign-in form. Two fields,
each with explicit labels, autocomplete hints, and helper text where it
adds value. Required-field indication is the \`required\` attribute plus
a visible asterisk in the label — aria-required alone does not meet
WCAG 3.3.2.
...
<div className="hx-narrative-hero">
  ...
  ></${dsName}-text-input>
  <div className="hx-narrative-hero-actions">
    <${dsName}-button variant="ghost">Forgot password?</${dsName}-button>
    <${dsName}-button variant="primary" type="submit">Sign in</${dsName}-button>
  </div>
</div>

## ARIA + keyboard contract

<APGPatternCard tag="${dsName}-text-input" />

## Consumer obligations

<ConsumerObligations
  tag="${dsName}-text-input"
  obligations={[
