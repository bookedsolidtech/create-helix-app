...
// ---------------------------------------------------------------------------
// _snippets.ts — code-string constants used by the MDX <CodeBlock>/<CodeTabs>
// instances. Lifted from helix/apps/storybook/stories/accessibility/_snippets.ts
// with two adaptations:
//
//  - Tag references in HTML/TS examples switch from `hx-*` to `{ds}-*`.
//  - The bash regenerate snippet drops the helix-internal pnpm-filter command
//    and replaces it with a neutral `npm run` placeholder so the consumer's
//    tokens-build script (whatever they wire) is the canonical entry point.
// ---------------------------------------------------------------------------

export function emitSnippetsTs({ dsName }: AccessibilityEmissionContext): AccessibilityMdxEmission {
  return {
    relativePath: 'src/stories/accessibility/_snippets.ts',
    content: `//
...
// which leaves CSS and JS comments alone.
//
...
export const DIALOG_HTML = \`<${dsName}-button id="open-prefs">Open preferences</${dsName}-button>

<${dsName}-dialog id="prefs" aria-labelledby="prefs-title">
  <h2 id="prefs-title" slot="header">Preferences</h2>

  <${dsName}-text-input label="Display name" autofocus></${dsName}-text-input>
  <${dsName}-text-input label="Email"></${dsName}-text-input>

  <${dsName}-button slot="footer" variant="ghost" data-dialog-close>Cancel</${dsName}-button>
  <${dsName}-button slot="footer" variant="primary" data-dialog-confirm>Save</${dsName}-button>
</${dsName}-dialog>\`;

export const DIALOG_TS = \`const trigger = document.querySelector<HTMLElement>('#open-prefs')!;
const dialog = document.querySelector<HTMLElement & { show(opts: unknown): void }>('#prefs')!;

trigger.addEventListener('click', () => {
  // The dialog records the trigger and traps focus on open …
 ...
 * Reactive controller pattern shared across tabs, menus, and radio groups.
 *
...
export const REGENERATE_CONTRAST_BASH = \`# Regenerate the contrast cache from your token cascade. The matrix
# component reads this report at build time, so a stale cache shows
# stale numbers in the deep-dive.
