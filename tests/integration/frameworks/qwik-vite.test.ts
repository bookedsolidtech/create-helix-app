...
describe('qwik-vite integration', () => {
  ...
  it('src/app.tsx imports from @builder.io/qwik', async () => {
    const o = opts('qv-app');
    await scaffoldProject(o);
    const app = await readText(o.directory, 'src/app.tsx');
    expect(app).toContain('@builder.io/qwik');
    expect(app).toContain('component$');
    expect(app).toContain('useSignal');
  });

  it('src/app.tsx imports helix-setup when designTokens is true', async () => {
    const o = opts('qv-app-tokens', { designTokens: true });
    await scaffoldProject(o);
    const app = await readText(o.directory, 'src/app.tsx');
    expect(app).toContain("import './helix-setup'");
  });

  it('src/app.tsx imports @helixui/library when designTokens is false', async () => {
    const o = opts('qv-app-no-tokens', { designTokens: false });
    await scaffoldProject(o);
    const app = await readText(o.directory, 'src/app.tsx');
    expect(app).toContain("import '@helixui/library'");
  });
