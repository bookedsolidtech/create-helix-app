...
# Usage: ./scripts/act-ci.sh [--job <job-name>] [--list] [--native] [--full] [--matrix] [--help]
#
...
show_help() {
  sed -n '3,20p' "${BASH_SOURCE[0]}" | sed 's/^# //' | sed 's/^#//'
  echo ""
  echo "Examples:"
...
if [[ "$USE_NATIVE" == true ]]; then
  ...
  ARCH_MODE="amd64 (Rosetta on Apple Silicon)"
else
  ARCH_ARGS="--container-architecture linux/amd64"
  ARCH_MODE="amd64 (native)"
fi

# -- Pnpm store mount --------------------------------------------------------
# Mount host pnpm store into container to avoid re-downloading packages
